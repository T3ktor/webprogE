...
function renderTable() {
    ...
    if (!tbody) return; // Biztonsági ellenőrzés
    ...
    data.forEach(item => {
        tbody.innerHTML += `
            <tr>
                ...
                <td>${item.orszag}</td>
                <td>
                    <button class="edit-btn" onclick="editItem(${item.id})">Szerkesztés</button>
...
function deleteItem(id) {
    if (!confirm("Biztosan törölni szeretnéd ezt a helységet?")) return;
...
// Kezdeti renderelés az oldal betöltésekor
